...
const cbest = myCategory.splice(1).find(bestB => (
                    ...
                    <div className=''>
                                <Link href={`/top-categories/${bestB.id.split(' ').join('_')}`} >
                                    <h2>{bestB.id}</h2>
                                </Link>
...
export default function page() {
    const topPage = myProducts.filter(selling => selling.category =='pro' )
    const selltPage = topPage.map(selling => (
                ...
                <div className=' grid place-items-center md:flex-col 2xl:gap-[40vw]  lg:grid-cols-2  w-[95%] m-auto  h-[90vh]'>
                    <div className=' pt-64 lg:pt-0'>
                        <div className='flex  text-purple-900  '>
                            <h1 className=' font-bold text-8xl xl:text-9xl'>{cbest.id}</h1>
                        </div>
                        <div className='flex '>
